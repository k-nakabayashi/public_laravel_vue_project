...
if ( isset( $_GET['oauth_token'] ) || isset($_GET["oauth_verifier"]) ) {
	...
	if (env('APP_ENV') !== 'local') {
		$url = 'http://homestead.test/register_Tw_Acccout';
	}
	
	$req_para = array(
		'app_id' => "",
		'user_id' => $query['user_id'],
		'user_id' => $query['screen_name'],
		'oauth_token' => $query['oauth_token'],
		'oauth_token_secret' => $query['oauth_token_secret'],
	);
	// $contents_array = post_request($req_url, $req_para);
...
// 認証画面から戻ってきた時 (認証NG)
	...
	// エラーメッセージを出力して終了
    // echo "連携を拒否しました。";
    echo "<script>alert('連携を拒否しました。');</script>";
...
$query_for_js = json_encode($query);
?>
